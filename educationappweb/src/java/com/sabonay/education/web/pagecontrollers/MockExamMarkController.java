 ...
 * @author Edwin
 */
@Named(value = "mockExamMarkController")
@SessionScoped
public class MockExamMarkController implements Serializable {

    private static final String MANAGED_BEAN_NAME = "mockExamMarkController";
    ...
    private ListDataModel<StudentMockExamMark> subjectStudentMarksList;
    ...
    public MockExamMarkController() {
        subjectStudentMarksList = new ListDataModel<StudentMockExamMark>();
        ...
        examMarkDirWrt = userData.getSchoolServerRootPath() + "mock-marks" + File.separator + "Empty Forms" + File.separator;
        examMarkDirRd = userData.getSchoolServerRootPath() + "mock-marks" + File.separator + "Submitted Forms" + File.separator;
    }

    public static MockExamMarkController getManagedInstance() {
        MockExamMarkController controller = (MockExamMarkController) JsfUtil.getManagedBean(MANAGED_BEAN_NAME);
    ...
    public void loadMarks() {
        ...
        List<StudentMockExamMark> list = LoadStudentMarks.loadClassStudentSubject_MockMarks(sc, subjectId, userData.getSchoolNumber() + "-" + selectedClass, userData);
        subjectStudentMarksList = new ListDataModel<StudentMockExamMark>(list);
    }
    ...
    public void loadStudentMarks() {
        ...
        List<StudentMockExamMark> list = LoadStudentMarks.loadClassStudentSubject_MockMarks(sc, subjectId, selectedClass, userData);
        subjectStudentMarksList = new ListDataModel<StudentMockExamMark>(list);
    ...
    public void updateStudentMarks() {
        ...
        try {
            StudentMockExamMark studentMark = (StudentMockExamMark) subjectStudentMarksList.getRowData();

            if (studentMark != null) {
                if (studentMark.getMockMark() == 0) {
                    studentMark.setMockMark(0.0);
                }
                studentMark.setAcademicYear(userData.getCurrentTermID());
                boolean updated = ds.getCommonDA().studentMockExamMarkUpdate(sc, studentMark);
    ...
    public void inputValueChange(ValueChangeEvent changeEvent) {
        ...
        StudentMockExamMark studentMark = subjectStudentMarksList.getRowData();
        if (updatedMarks > 100) {
        ...
        if (componentId.contains("mock")) {
            studentMark.setMockMark(updatedMarks);
            studentMark.setAcademicYear(userData.getCurrentTermID());
            ds.getCommonDA().studentMockExamMarkUpdate(sc, studentMark);
    ...
    public void updateAllStudentMarks() {
        SabonayContext sc = SabonayContextUtils.getSabonayContext();
        for (StudentMockExamMark studentMockExamMark : subjectStudentMarksList) {
            studentMockExamMark.setAcademicYear(userData.getCurrentTermID());
            ds.getCommonDA().studentMockExamMarkUpdate(sc, studentMockExamMark);
        }
    }
    ...
    public void writeExcel() {
        ...
        reportTitle = className + " - " + selectedSubject + " - " + acdemicTerm + " - MOCK.xlsx";
        ...
        ffl.mkdirs();
        File ffll = new File(System.getProperty("user.home") + "/Desktop/SABONAY/MOCK/");
        ffll.mkdirs();
        try {
            File file = new File(examMarkDirWrt, System.getProperty("user.home") + "/Desktop/SABONAY/MOCK/GENERATED EMPTY SHEETS.lnk");
            if (!file.exists()) {
                ShellLink.createLink(examMarkDirWrt, System.getProperty("user.home") + "/Desktop/SABONAY/MOCK/GENERATED EMPTY SHEETS.lnk");
            }
        } catch (IOException ex) {
            Logger.getLogger(StudentMarkController.class.getName()).log(Level.SEVERE, null, ex);
        }
        try {
            File file = new File(examMarkDirRd, System.getProperty("user.home") + "/Desktop/SABONAY/MOCK/SUBMITTED EXCEL SHEETS.lnk");
            if (!file.exists()) {
                ShellLink.createLink(examMarkDirRd, System.getProperty("user.home") + "/Desktop/SABONAY/MOCK/SUBMITTED EXCEL SHEETS.lnk");
            }
        } catch (IOException ex) {
            Logger.getLogger(StudentMarkController.class.getName()).log(Level.SEVERE, null, ex);
        }
        ...
        XSSFSheet sheet = workbook.createSheet("MOCK-EXAM Marks - " + reportTitle + " " + subjectTeacher);
...
//        sheet.addValidationData(validation2);
        ...
        cell.setCellValue("MOCK EXAM SCORE");
        cell.setCellStyle(style);
...
//            } else {
            ...
            workbook.write(out);
            out.close();
            JsfUtil.addInformationMessage("Excel file for " + className + " " + selectedSubject + " - MOCK successfully generated");
//            }
    ...
    public void readExcel() {
        try {
            ...
            if (!ff.exists()) {
                JsfUtil.addErrorMessageAndRespond("Excel sheet for " + className + " " + selectedSubject + " - MOCK has not been submitted");
            } else {
...
//                    while (cellIterator.hasNext()) {
                    ...
                    for (StudentMockExamMark sm : subjectStudentMarksList) {
                        if (stdId.equals(sm.getStudent().getStudentFullId())) {
                            StudentMockExamMark studentMark = sm;
...
//                            }
    ...
    public boolean valueChangeExcel(double exammark, StudentMockExamMark studentMark) {
        try {
            SabonayContext sc = SabonayContextUtils.getSabonayContext();
            studentMark.setMockMark(exammark);
            boolean examUpdate = ds.getCommonDA().studentMockExamMarkUpdate(sc, studentMark);
    ...
    public ListDataModel<StudentMockExamMark> getSubjectStudentMarksList() {
        return subjectStudentMarksList;
    }

    public void setSubjectStudentMarksList(ListDataModel<StudentMockExamMark> subjectStudentMarksList) {
