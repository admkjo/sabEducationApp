 ...
 * @author Edwin
 */
public class StudentBroadSheet {

    private List<ExamMarkDetail> listOfStudentMarkDetail4Rpts = null;
    private ExamMarkDetail[] marksDetail;
    private StudentReportDetail examReportDetail = null;
    private int counter;
    ...
    public StudentBroadSheet() {
        listOfStudentMarkDetail4Rpts = new LinkedList<ExamMarkDetail>();
        examReportDetail = new StudentReportDetail();
    }
    ...
    public List<ExamMarkDetail> getListOfStudentMarkDetail4Rpts() {
        return listOfStudentMarkDetail4Rpts;
    }

    public void setExamMarksDetailList(List<ExamMarkDetail> listOfStudentMarkDetail4Rpts) {
        this.listOfStudentMarkDetail4Rpts = listOfStudentMarkDetail4Rpts;

        marksDetail = listOfStudentMarkDetail4Rpts.toArray(new ExamMarkDetail[listOfStudentMarkDetail4Rpts.size()]);
        //String stdId = listOfStudentMarkDetail4Rpts.get(0).getStudentId();
        //String acaYear = "2013/2014";
//        System.out.println("THE STD ID IS "+stdId);
        //Student s = ds.getCommonDA().studentFind("0050705-KASH12718");
        ...
        for (ExamMarkDetail studentMarkDetail : listOfStudentMarkDetail4Rpts) {
            ...
            if (studentMarkDetail.getExaminationType() == ExaminationType.MOCK_EXAMINATION) {

                try {
                    double totalMarks = Double.parseDouble(studentMarkDetail.getTotalScore());
                    if (totalMarks < 40) {
                        --numberOfPassesInt;
                    }
                } catch (Exception e) {
            ...
            if (studentMarkDetail.getExaminationType() == ExaminationType.MID_TERM_EXAMINATION) {
...
//        if (listOfStudentMarkDetail4Rpts.get(0).getExaminationType() == ExaminationType.MID_TERM_EXAMINATION) {
//            numberOfPassesInt = ds.getCustomDA().numberOfStudentPasses(s.getStudentFullId(),acaYear, 40);
//        } else {
//      
//        }
    ...
    public String getSubjectTotalForSubjectCode(String subjectCode) {
        ...
        for (ExamMarkDetail studentMarkDetail4Rpt : listOfStudentMarkDetail4Rpts) {
            if (studentMarkDetail4Rpt == null) {
                continue;
            }

            if (studentMarkDetail4Rpt.getSubjectCode() == null) {
                continue;
            }

            if (studentMarkDetail4Rpt.getSubjectCode().equalsIgnoreCase(subjectCode)) {
                subjectTotalMarks = studentMarkDetail4Rpt.getTotalScore();

                if (subjectTotalMarks.equalsIgnoreCase("-")) {
                    subjectTotalMarks = xEduConstants.NO_MARKS;
                }
            }
        }
    ...
    public String getMarkForSubject(String subjectCode) {
        ...
        ExamMarkDetail markDetail = getMarkDetail(subjectCode);
        if (markDetail != null) {
            return markDetail.getTotalScore();
        }

        return "";
    }

    public ExamMarkDetail getMarkDetail(String subjectCode) {
        for (ExamMarkDetail studentMarkDetail : listOfStudentMarkDetail4Rpts) {
            if (studentMarkDetail.getSubjectCode() != null) {
                if (studentMarkDetail.getSubjectCode().equalsIgnoreCase(subjectCode)) {
    ...
    public String getMarkDescription(String subjectCode) {
        ExamMarkDetail markDetail = getMarkDetail(subjectCode);
        if (markDetail != null) {
//            String desc = "CM: " +markDetail.getClassScore() +","
//                    +"EM : " +markDetail.getExamScore() +","
//                    +"E: " + markDetail.getGrades() + ","
//                    +"Post: " + markDetail.getPositionInClass() + "";
//            return desc;
        }
        return "";
    }
